<template>
  <div class="q-pa-xl">
    <q-breadcrumbs class="qb cursor-pointer q-pb-md">
      ...
      />
      <q-breadcrumbs-el label="Otras Variantes" />
    </q-breadcrumbs>
    <q-table
      class="q-pa-md"
      :filter="filter"
      :rows="items"
      :columns="dataColumnOtraVariante"
      row-key="id"
            ...
            <header class="q-pa-sm bg-primary">
              <q-toolbar>
                <q-toolbar-title class="text-subtitle6 text-white">
                  {{
                    objeto.id ? "Editar Otra Variante" : "Adicionar Otra Variante"
                  }}</q-toolbar-title
                >
              </q-toolbar>
            </header>
...
import { dataColumnOtraVariante } from 'src/assets/js/column_data/columnDataNomencladores'
...
const Guardar = () => {
  const url = objeto.id ? 'OtraVariante/Actualizar' : 'OtraVariante/Crear'
  saveData(url, objeto, load, close, dialogLoad)
}

// Funcion para Obtener los datos para editar
const obtenerElementoPorId = async (id) => {
  await obtener('OtraVariante/ObtenerPorId', id, objeto, dialogLoad, dialog)
}

// Funcion para eliminar elemento
const eliminar = async () => {
  await eliminarElemento(
    'OtraVariante/Eliminar',
...
const load = async () => {
  items.value = await loadGet('OtraVariante/ObtenerListadoPaginado')??[]
}
